...
<script>
	...
	export default {
		...
		methods: {
			...
			remoteMethod(query) {
				if (query !== '') {
					this.$http.get(`asset/api/nro/network/network?device_type=FW&query=${query}&is_monitor=是`, {
						headers: {
							'token': localStorage.getItem('token')
						}
					}).then((res) => {
			...
			serveHistory(start, end) {
				this.$http.get(
					`monitor/api/network_monitor/monitor/?network_name=FIREWALL_${this.networkIp}_${this.deviceName}`, {
						headers: {
							'token': this.token
						}
					}).then((res) => {
