			...
			<el-row :span="24" class="mlr10 mt10">
				<el-col :span="16">
					<div class="mb10">
								...
								<el-col :span="7">
									<el-form-item label="IP" label-width="50px">
										...
										class="select_width" @change="changeIp">
											<el-option v-for="(item) in optionsNetworkIp" :key="item.id" :label="item.ipaddr" :value="item.ipaddr+'/'+item.hostname">
											</el-option>
										</el-select>
									</el-form-item>
								</el-col>
						...
						<el-row>
							<el-col :span="12" v-show="isShowName">
								<div class="network_top_top">
									<div>
										<dl class="top_font_10 mlr10">
											<dt class="serve_name">{{deviceName}}</dt>
											<dd>设备名称</dd>
										</dl>
									</div>
								</div>
				...
				<el-col :span="8">
					<div class="network_top ml10 mb10">
						...
						<el-table class="table-content" :data="portStatusData" border height="200px" style="">
							<el-table-column  prop="name" label="端口名称" align="center">
							</el-table-column>
							<el-table-column  prop="value" label="端口状态" align="center">
								<template slot-scope="scope">{{scope.row.value==1 ? 'up' :'down'}}</template>
							</el-table-column>
						</el-table>
					</div>
				</el-col>
...
<script>
	...
	export default {
		...
		methods: {
			queryOneIp() {
				this.$http.get(`asset/api/nro/network/?type=LB&current_page=1&pre_page=1&is_monitor=是`, {
					headers: {
						'token': localStorage.getItem('token')
					}
				}).then((res) => {
					this.IP = res.data.data[0].ipaddr;
					this.networkIp = res.data.data[0].ipaddr;
					this.deviceName = res.data.data[0].hostname;
					const curTime = Date.parse(this.networkTime[1]) / 1000;
					const startTime = Date.parse(this.networkTime[0]) / 1000;
					this.optionsData = [];
					this.serveHistory(startTime, curTime);
			...
			remoteMethod(query) {
				if (query !== '') {
					this.$http.get(`asset/api/nro/network/network?device_type=LB&query=${query}&is_monitor=是`, {
						headers: {
							'token': localStorage.getItem('token')
						}
					}).then((res) => {
			...
			serveHistory(start, end) {
				this.$http.get(
					`monitor/api/network_monitor/monitor/?network_name=LOADBALANCE_${this.networkIp}_${this.deviceName}`, {
						headers: {
							'token': this.token
						}
					}).then((res) => {
