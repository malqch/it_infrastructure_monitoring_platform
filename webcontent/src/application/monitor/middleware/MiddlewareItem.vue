        ...
        <div class="wrapper">
            ...
            <el-dialog :title="title" :visible.sync="editVisible" width="30%">
                <el-form ref="form" :rules="rules" :model="form" label-width="93px">
                    <el-form-item label="中间件类型" prop="db_type">
            ...
            <el-dialog title="提示" :visible.sync="deleteVisible" width="25%">
                <span>确定要删除 {{ middleware_type}} - {{item }} 中间件监控项信息吗？</span>
                <span slot="footer" class="dialog-footer">
    ...
    export default {
        data() {
            return {
                ...
                rules: {
                    ...
                    db_type: [
                        { required: true, message: '请输入中间件类型', trigger: 'blur' },
                    ]
                },
                middleware_type: '',
                item: ''
        ...
        methods: {
            ...
            getData() {
                ...
                let url = this.url;
                if (keyword) {
                    url=`monitor/api/middleware_item/?item_name=${keyword}&current_page=${this.query.pageIndex}&pre_page=${this.query.pageSize}`
                }else {
                    url=`monitor/api/middleware_item/?current_page=${this.query.pageIndex}&pre_page=${this.query.pageSize}`
                }
                this.$http.get(`${url}`, {
                    headers: {
            ...
            handleDelete(index, row) {
                this.id = row.id;
                this.form = row;
                this.middleware_type = row.middleware_type;
                this.item = row.item;
                this.deleteVisible = true;
            },
            // 删除操作
            deleteDBItem() {
                this.$http.delete(`monitor/api/middleware_item/${this.id}/`,
                    {
                        headers:{
                            'token': localStorage.getItem('token')
                        }
            ...
            saveEdit() {
                if(this.title === '编辑') {
                    this.$http.put(`monitor/api/middleware_item/${this.id}/`,
                        this.form,
                        {
                            headers:{
                                'token': localStorage.getItem('token')
                    ...
                    this.$http.post(`monitor/api/middleware_item/`,
                        this.form,
                        {
                            headers:{
                                'token': localStorage.getItem('token')
