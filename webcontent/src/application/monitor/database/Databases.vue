        ...
        <div class="wrapper">
            <div class="container">
                ...
                >
                    ...
                    <el-table-column prop="name" label="数据库名称" align="center" width="120"></el-table-column>
                    <el-table-column prop="db_type" label="数据库类型" align="center" width="120"></el-table-column>
                    <el-table-column prop="ip_address" label="数据库IP" align="center"></el-table-column>
                    <el-table-column prop="physicalip" label="物理IP" align="center"></el-table-column>
                    <el-table-column prop="sid" label="SID" align="center"></el-table-column>
            ...
            <el-dialog :title="title" :visible.sync="editVisible" width="30%">
                <el-form ref="form" :rules="rules" :model="form" label-width="100px">
                    ...
                    <el-form-item label="物理IP" prop="physicalip">
                        <el-input v-model="form.physicalip"></el-input>
                    </el-form-item>
            ...
            <el-dialog title="提示" :visible.sync="deleteVisible" width="25%">
                <span>确定要删除 {{ db_name }} 数据库信息吗？</span>
                <span slot="footer" class="dialog-footer">
    ...
    export default {
        data() {
            return {
                ...
                rules: {
                    ...
                    physicalip: [
                        { required: true, message: '请输入数据库物理地址', trigger: 'blur' },
                    ],
                    sid: [
                        { required: true, message: '请输入数据SID', trigger: 'blur' },
                    ]
                },
                db_name: '',
            };
        ...
        methods: {
            ...
            getData() {
                ...
                let url = this.url;
                if (keyword) {
                    url=`monitor/api/databases/?name=${keyword}&current_page=${this.query.pageIndex}&pre_page=${this.query.pageSize}`
                }else {
                    url=`monitor/api/databases/?current_page=${this.query.pageIndex}&pre_page=${this.query.pageSize}`
                }
                    this.$http.get(`${url}`, {
                        headers: {
            ...
            handleDelete(index, row) {
                this.id = row.id;
                this.form = row;
                this.db_name = row.name;
                this.deleteVisible = true;
            },
            // 删除操作
            deleteDB() {
                this.$http.put(`monitor/api/databases/delete/`,
                    {'ids': [this.id]},
                    {
                        headers:{
                            'token': localStorage.getItem('token')
            ...
            saveEdit() {
                if(this.title === '编辑') {
                    this.$refs.form.validate((valid) => {
                            if (valid) {
                                this.$http.put(`monitor/api/databases/${this.id}/`,
                                    this.form,
                                    {
                                        headers:{
                                            'token': localStorage.getItem('token')
                    ...
                    this.$refs.form.validate((valid) => {
                            if (valid) {
                                this.$http.post(`monitor/api/databases/`,
                                    this.form,
                                    {
                                        headers:{
                                            'token': localStorage.getItem('token')
