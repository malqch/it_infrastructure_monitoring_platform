<template>
	<div class="app-container">
		<div id="torpBox_test" class="main_view" />
		<div class="l000_mask">
			<div class="l000_win_device">
				<h5>设备信息</h5>
				<span class="l000_win000_close">+</span>
				<ul id="l000_mach000_info" class="l000_mach000_info">
					<li>设备ID：<span id="machId2" /></li>
					<li>设备型号：<span id="machType2" /></li>
					<li>设备类型：<span id="machTypeName2" /></li>
					<li>设备IP：<span id="machIp2" /></li>
				</ul>
...
<script>
	//import { fetch2D_physicsLogicTopology_2 } from '@/api/operation'
export default {
	...
	methods: {
		getTorpData(webResData) {
			...
			webResData.topology[0].node.forEach((v, i) => {
				const limitRow = 5
				webRslData.nodes.push({
			...
			webResData.topology[0].link.forEach((v, i) => {
				if (v['bandwidth'] === undefined) {
					v['bandwidth'] = ''
				}
				webRslData.links.push({
					'id': '1000_line000_' + new Date().getTime() + '' + i,
					'name': '',
					...
					'diy_attr': {
						'link-id': v['link-id'],
						'all_data': this.webResData,
						'bandwidth': v['bandwidth'],
						'bandwidth-used': v['bandwidth-used'],
						'status': v['status'],
						'all_data_init': this.all_data_init
					}
				})
			})
			return webRslData
			console.log(webRslData)
		},
		getBkgd(type, gType) {
			if (type === 'defualt') {
				if (gType === 'stack') {
		...
		getMapList() {
			...
			this.$http.get(`phy_topo/hlw_node_topo`, {
				headers: {
				...
				if (response.data != null) {
					if (response.data.msgcode === 0) {
						...
						this.webDataFromRequest = JSON.parse(JSON.stringify(response.data.data));
						for (let index = 0; index < this.webDataFromRequest.topology[0].node.length; index++) {
							const element = this.webDataFromRequest.topology[0].node[index]
							if (!Object.prototype.hasOwnProperty.call(element, "device-family")) {
								this.webDataFromRequest.topology[0].node[index]['device-family'] = 'defualt.000000'
							}
							if (!Object.prototype.hasOwnProperty.call(element, "devicegroup-type")) {
								this.webDataFromRequest.topology[0].node[index]['devicegroup-type'] = 'defualt'
							}
							if (!Object.prototype.hasOwnProperty.call(element, "device-type")) {
								this.webDataFromRequest.topology[0].node[index]['device-type'] = 'defualt'
							}
							if (!Object.prototype.hasOwnProperty.call(element, "connection-type")) {
								this.webDataFromRequest.topology[0].node[index]['connection-type'] = 'defualt'
							}
							if (!Object.prototype.hasOwnProperty.call(element, "host")) {
								this.webDataFromRequest.topology[0].node[index]['host'] = '0.0.0.0'
							}
							if (!Object.prototype.hasOwnProperty.call(element, "status")) {
								this.webDataFromRequest.topology[0].node[index]['status'] = 'defualt'
							}
						}
						this.webDataFromRequest.topology[0].node.forEach((v) => {
							const reg = new RegExp('\\.', 'g')
							v['node-id'] = v['node-id'].replace(reg, '-')
							const reg2 = new RegExp(':', 'g')
							v['node-id'] = v['node-id'].replace(reg2, '-')
						})
						this.webDataFromRequest.topology[0].link.forEach((v) => {
							const reg = new RegExp('\\.', 'g')
						...
						this.webResData = this.webDataFromRequest;
						
						//console.count()
						const _self = this;
						const torpMap_2 = new TopologyMap2({
							mainBoxId: 'torpBox_test',
							canvasId: 'canvas_2',
							couldEdit: false,
							onlyDrag: true,
							leftViewShow: false,
						...
						//console.log(_self.getTorpData(_self.webResData))
						torpMap_2.setTorpData(_self.getTorpData(_self.webResData));
						$('.l000_torp000_element').on('click', function() {
							if ($(this).attr('data-win')) {
								let data = JSON.parse($(this).attr('data-win'));
								
								$('#machId2').text(data.machId)
								if (data.machType === 'undefined()') {
									$('#machType2').text('')
								} else {
									$('#machType2').text(data.machType)
								}
								$('#machTypeName2').text(data.machTypeName)
								if (data.host === '0.0.0.0') {
									$('#machIp2').text('')
								} else {
									$('#machIp2').text(data.host)
								}
								...
								if (data.machTypeName == "unknow" || data.host == "") {
									...
									if (data.machTypeName == 'Switch' || data.machTypeName == 'Router') {
										...
										$('.btn_control')
											.attr('href', localhostPaht[0] + '#/monitor/network_visualize?ip=' + data.host + '&device_type=' + data
												.machTypeName + '&name=' + data.machId) //
											.attr('target', '_blank');
										$('.btn_control').css({
											'background-color': '#409EFF'
										}).attr('disabled', false)
...
<style>
	...
	html {
		font-family: Tahoma, Arial, Roboto, ”Droid Sans”, ”Helvetica Neue”, ”Droid Sans Fallback”, ”Heiti SC”, sans-self, Microsoft YaHei;
	...
	* {
	...
	.l000_torp000_element>.l000_drag000_element {
		margin: 0;
	}
	...
	.l000_torp000_info000_window>p {
		margin-bottom: 18px;
	}

	.l000_torp000_element.l000_left000_text,
	.l000_torp000_element.l000_left000_text>div {
		width: auto;
		height: auto;
		text-align: center;
		border: none;
	...
	.l000_torp000_dot:hover {
		border: solid 2px #3d7fbb;
	}

	.l000_torp000_dot.l000_dot000_active {
		border: solid 2px #3d7fbb;
		background: #3d7fbb;
	}
	...
	.l000_right000_box>ul,
	.l000_left000_drag000_box {
		width: 100%;
		height: auto;
		overflow: hidden;
		min-width: 200px;
	...
	.l000_title.l000_open::after {
		transform: rotate(45deg);
		top: 10px;
	}

	.l000_element000_box label,
	.l000_line000_box label {
		font-size: 12px;
		color: #666;
	}

	.l000_element000_box li,
	.l000_line000_box li {
		margin-top: 20px;
		padding: 0 4%;
		box-sizing: border-box;
	...
	.l000_element000_box li.l000_nomargin,
	.l000_line000_box li.l000_nomargin {
		margin-top: 0px;
	}

	.l000_diy000_style>div {
		width: 160px;
		height: 160px;
		background: #fff;
	...
	.l000_diy000_style>div p {
		font-size: 14px;
		text-align: center;
	}

	.l000_diy000_style>div span {
		display: block;
		background: #afafaf;
		width: 100px;
		height: 100px;
	...
	.l000_diy000_input000_box {
		width: 100%;
		position: relative;
	}

	.l000_diy000_input000_box input {
		width: 40%;
	}

	.l000_add000_diy000_style p {
		display: block;
		position: relative;
		height: 20px;
		cursor: pointer;
		border: solid 1px #53adff;
		color: #53adff;
		width: 100%;
		padding: 4px 0;
		border-radius: 4px;
	...
	.l000_diy000_color input {
		width: 120px;
	}

	.l000_diy000_color>div {
		margin-top: 20px;
	}
	...
	.l000_tool000_btn.l000_left000_active {
		color: #53adff;
		border: solid 1px #53adff
	}
